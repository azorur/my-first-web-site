...
public class _04_InheritBoard {
	...
	public static void main(String[] args) {
		...
		FreeBoard freeBoard = new FreeBoard(2,"자유게시판 제목","자유게시판 내용","홍길동","231020","java.jpg");
		
		NoticeBoard noticeboard = new NoticeBoard(3,"공지게시판 제목","공지게시판 내용","관리자","231020",true);
		
		//2. 자식클래스를 부모타입으로 사용할 때는 자식 클래스의 고유 속성이나 기능을 사용하지 못한다.
//		board.upload("파일제목");
		freeBoard.upload("파일제목");
		
		//3. 자식클래스를 부모타입으로 사용하면 하나의 변수로 여러가지 자식클래스를 사용할 수 있다. 다양한 형태로 사용할 수 있기 때문에 다형성이라고 부른다.
		//Board 타입 변수에 FreeBoard 객체를 담아서 공통기능인 insertBoard()가 실행된다.
